...
public class PlayerMove : PhysicsBase, IFPlayerMove, IFLandingEvent
{
    #region フィールド変数
    ...
    private void Update()
    {
        ...
        // 落下処理
        _fall.FallObject(CheckFloor.CheckHitFloor(), CheckCeiling.CheckHitCeiling());
    ...
    public void PlayerAttack()
    {
        ...
        if (_playerState.GetState(E_PlayerState.Attack) && _attackInterval <= 0)
        {
            ...
            {
                // 予約を消す
                StopCoroutine(_attackFlagCoroutine);
            }

            // 攻撃が終わったらオフにする予約を入れる
            _playerState.OffStateFlag(E_PlayerState.Attack, ATTACK_STIFFENING);
    ...
    public void PlayerHealStart()
    {
        ...
        if (_playerState.GetState(E_PlayerState.Heal) && CheckFloor.CheckHitFloor() && !_playerParameter.IsHPMax())
        {
            // 消費SPが足りているか確認
