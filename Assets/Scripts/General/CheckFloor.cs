...
public class CheckFloor
{
    ...
    public bool CheckHitFloor()
    {
        ...
        {
            // オブジェクトと接触しているか判定
            if(Physics.Raycast(_thisTransform.position + _objectButtom * Vector3.down + (-_objectDepth + _checkDistance * i) * Vector3.right, Vector3.down, rayLength, groundLayer))
            {
                // ぶつかっている場合
                return true;
            }
